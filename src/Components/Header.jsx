    ...
    return (
        <header className="bg-[#141414] text-white p-6 flex items-center justify-between px-6 md:px-12 shadow-lg fixed w-full top-0 z-50  backdrop-blur-md">
            ...
            <nav className={`absolute md:static top-full left-0 w-full md:w-auto bg-[#3D444B] md:bg-transparent flex flex-col md:flex-row gap-6 md:gap-12 p-6 md:p-0 transition-all duration-300 ${menuOpen ? 'block' : 'hidden md:flex'}`}>
                ...
                <a href="#" className="text-[#ABACAD] hover:text-[#D7263D] transition duration-300">Каталог</a>
            ...
            <div className="flex items-center gap-4 md:gap-6 relative">
                <motion.div animate={{ width: searchOpen ? "14rem" : "0rem", opacity: searchOpen ? 1 : 0 }}
                            transition={{ duration: 0.4, ease: "easeInOut" }}
                            className="absolute right-12 md:right-20 bg-[#ABACAD]] text-[#ABACAD]] border border-[#AEC0C2] rounded-lg overflow-hidden shadow-lg">
                        <input
                            type="text"
                            placeholder="Поиск..."
                ...
                />
                <User className="cursor-pointer text-[#ABACAD]] hover:text-[#D7263D] transition duration-300 transform hover:scale-125"   onClick={() => navigate("/регистрация")}/>
